...
function getNavItems(
 ...
 { id: "messages", label: "Messages", icon: MessageSquare, href: "/messages", badge: unreadMessages > 0 ? unreadMessages : undefined },
...
export function GlassmorphismSidebar({
 ...
 const { unreadCount: unreadMessages } = useUnreadCount(profile?.id)
 const navItems = getNavItems(userRole, pendingCount, unreadNotifications, unreadMessages)
 ...
 boxShadow: "-4px 0 40px rgba(0,0,0,0.6), 0 4px 20px rgba(0, 0, 0, 0.3)",
 display: "flex",
 ...
 <span
 className={cn(
 "absolute left-0 w-0.5 h-5 rounded-r-full bg-gradient-to-b from-[#0D6F73] to-[#0D6F73] transition-all duration-200",
 ...
 active ? "text-[#0D6F73]" : "text-white/40 group-hover:text-white/70"
 )}
 />
 ...
 <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#0D6F73] to-[#0D6F73] text-xs font-bold text-white">
 {userInitials}
 </span>
 )}
 ...
 <p className="text-[11px] text-[#0D6F73] font-medium mt-0.5 truncate">
 {profile.email}
 </p>
 </div>
 </div>
