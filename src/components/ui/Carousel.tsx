...
function CarouselItem({
 ...
 : "items-start bg-card border border-border rounded-2xl"
 }`}
 style={{
 width: itemWidth,
 ...
 : "linear-gradient(135deg, #0D6F73 0%, #0D6F73 100%)" 
 }}
 >
 {item.icon && (
 <span className="carousel-icon-pulse flex h-12 w-12 items-center justify-center rounded-full bg-card border border-border text-white z-10">
 {item.icon}
 </span>
 )}
 {item.tour_type && (
 <span className="absolute top-3 left-3 text-[9px] uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full bg-card shadow-modern text-white border border-white/15 z-10">
 {item.tour_type}
 </span>
 )}
 ...
 {item.hostName && (
 <span className="text-xs text-[#0D6F73] font-semibold truncate max-w-[90px]">
 {item.hostName}
 </span>
 )}
 ...
 <span className="text-[#0D6F73]">{item.icon}</span>
 </div>
 )}
 </div>
 </motion.div>
...
export default function Carousel({
 ...
 : "rounded-[24px] border border-border bg-card shadow-modern shadow-xl"
 }`}
 style={{
 width: `${baseWidth}px`,
