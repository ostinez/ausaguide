...
export function HeroSection() {
  ...
  // Fetch real tours & community posts from Supabase via useWallItems hook
  const { items: wallItems } = useWallItems()

  // Track responsive screen size for dynamic DriftWall layout
  useEffect(() => {
  ...
  // Intersection observer for optimal background animation performance
  useEffect(() => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden bg-[#06363D] w-full min-h-[500px] h-[580px] sm:h-[640px] md:h-[700px] lg:h-[760px]"
    >
      {/* Background Animated DriftWall Layer */}
      ...
      {/* Radial vignette overlay for maximum foreground contrast & readability */}
      <div
