...
export function Layout() {
 ...
 if (maintenanceMode && userRole !== "admin" && location.pathname !== "/auth") {
    return (
      <div className="flex min-h-svh flex-col items-center justify-center bg-background text-foreground px-4 relative overflow-hidden">
        <div className="max-w-md w-full text-center space-y-8 z-10 p-8 rounded-3xl border border-border bg-card shadow-modern">
          ...
          <div className="pt-6 border-t border-border flex flex-col items-center justify-center gap-2">
            ...
            <Link to="/auth" className="text-xs text-brand font-semibold hover:underline mt-4 transition-colors">
              Admin Login Bypass
            </Link>
          </div>
        </div>
 ...
 role="alert"
 aria-live="assertive"
 className="fixed bottom-6 left-1/2 -translate-x-1/2 max-w-sm w-[90%] bg-red-950/95 border border-red-500/30 text-red-200 px-4 py-3 rounded-xl shadow-2xl z-50 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-5"
 >
