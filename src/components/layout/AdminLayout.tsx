...
export function AdminLayout() {
 ...
 <div className="absolute top-[-20%] left-[-10%] h-[600px] w-[600px] rounded-full bg-brand/15 blur-[120px]" />
 <div className="absolute bottom-[-10%] right-[-10%] h-[500px] w-[500px] rounded-full bg-emerald-600/5 blur-[120px]" />
 </div>

 {/* Mobile Header */}
 <header className="md:hidden flex items-center justify-between px-6 py-4 border-b border-white/5 bg-card sticky top-0 z-40 w-full">
 ...
 {/* Sidebar Navigation */}
 <aside className={`
 fixed inset-y-0 left-0 z-30 w-64 border-r border-white/5 bg-card transform md:transform-none transition-transform duration-300 ease-in-out flex flex-col pt-16 md:pt-0
 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
 `}>
 {/* Logo area */}
 <div className="hidden md:flex items-center gap-3 px-6 py-8">
 <div className="flex size-11 items-center justify-center rounded-xl bg-brand/15 border border-brand/30 text-brand">
 ...
 {/* User Info */}
 <div className="px-6 py-4 border-b border-white/5 bg-card shadow-modern mb-4 mx-3 rounded-xl flex items-center gap-3">
 <div className="size-8.5 rounded-full bg-brand/15 flex items-center justify-center text-xs font-bold text-brand uppercase">
 {adminName.slice(0, 2)}
 </div>
 ...
 ${isActive 
 ? "bg-brand/15 text-brand border-brand/30 font-bold" 
 : "text-white/60 hover:text-white hover:bg-card shadow-modern hover:border-white/5"}
 `}
 >
