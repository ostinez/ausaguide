...
export default function ThankYouPage() {
 ...
 {/* Checkmark icon */}
 <div className="mx-auto flex size-20 items-center justify-center rounded-full bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon/10 border border-[#0D6F73]/30 ring-4 ring-[#0D6F73]/10 animate-pulse">
 ...
 <GradientText
 colors={isTree ? ["#0D6F73", "#FFFFFE"] : ["#0D6F73", "#FFFFFE"]}
 animationSpeed={4.5}
 yoyo={true}
 >
 Thank You!
 ...
 {sessionId && (
 <SpotlightCard className="p-6 border border-border bg-card shadow-modern rounded-2xl text-left space-y-4">
 <h3 className="text-xs font-black uppercase tracking-wider text-white/30 flex items-center gap-1.5">
 <ShieldCheck className="size-3.5 text-[#0D6F73]" />
 ...
 <span className="font-bold text-[#0D6F73]">{formattedAmount}</span>
 </div>
 <div className="flex justify-between border-b border-border pb-2">
 <span className="text-white/45">Status</span>
 <span className={`font-bold capitalize ${donation.status === "completed" ? "text-[#0D6F73]" : "text-amber-400"}`}>
 {donation.status}
 </span>
 </div>
