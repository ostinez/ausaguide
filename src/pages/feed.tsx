...
function LikeListModal({ postId, likeCount, likers, onClose }: {
 ...
 <div key={liker.user_id} className="flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-card shadow-modern transition-colors">
...
function CreatePostCard({ currentUserId, onCreated }: { currentUserId: string; onCreated: (p: Post) => void }) {
 ...
 return (
 <div className="rounded-2xl border border-border/60 bg-card shadow-modern p-4 space-y-3">
 <textarea
 id="new-post-input"
 ...
 id="post-submit-btn"
 size="sm"
 className="rounded-full px-5 bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon hover:bg-[#6b47d6] text-white text-xs font-semibold"
...
function ImageCarousel({ urls, onImageClick }: { urls: string[]; onImageClick: (urls: string[], index: number) => void }) {
 ...
 return (
 <div className="relative group overflow-hidden rounded-xl border border-white/5 bg-card shadow-modern shadow-inner aspect-video">
 {/* Embla Viewport */}
 ...
 className="absolute left-3 top-1/2 -translate-y-1/2 z-20 flex size-8 items-center justify-center rounded-full border border-white/10 bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/85 hover:scale-105 active:scale-95"
 >
 ...
 className="absolute right-3 top-1/2 -translate-y-1/2 z-20 flex size-8 items-center justify-center rounded-full border border-white/10 bg-black/60 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/85 hover:scale-105 active:scale-95"
 >
 ...
 {/* Indicators */}
 {urls.length > 1 && (
 <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex gap-1.5 px-2.5 py-1 rounded-full bg-black/40 ">
 {urls.map((_, idx) => (
 <button
 key={idx}
 type="button"
 ...
 idx === selectedIndex ? "w-3 bg-primary" : "bg-card shadow-modern hover:bg-card shadow-modern"
 )}
 />
 ))}
 </div>
...
function ImageLightbox({ urls, initialIndex, onClose }: { urls: string[], initialIndex: number, onClose: () => void }) {
 ...
 return (
 <div className="fixed inset-0 z-[150] bg-black/95 flex flex-col" onClick={onClose}>
 <div className="flex justify-between items-center p-4">
 <span className="text-white text-sm font-medium">{currentIndex + 1} / {urls.length}</span>
 <button onClick={onClose} className="p-2 bg-black/50 hover:bg-card shadow-modern rounded-full text-white transition-colors">
 <X className="size-5" />
 </button>
 </div>
 ...
 <>
 <button 
 className="absolute left-4 p-3 bg-black/50 hover:bg-card shadow-modern rounded-full text-white transition-colors"
 onClick={(e) => { e.stopPropagation(); setCurrentIndex(prev => (prev > 0 ? prev - 1 : urls.length - 1)) }}
 >
 <ChevronLeft className="size-6" />
 </button>
 <button 
 className="absolute right-4 p-3 bg-black/50 hover:bg-card shadow-modern rounded-full text-white transition-colors"
...
const PostCard = memo(function PostCard({
 ...
 return (
 <div className="post-card rounded-2xl border border-white/10 bg-card shadow-modern p-4 space-y-3 shadow-[0_8px_32px_rgba(0,0,0,0.37)] hover:border-primary/30 transition-all">
 {/* Author */}
 ...
 <span className="inline-flex items-center gap-1 text-[9px] font-bold text-[#b49cf8] bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon/10 border border-[#0D6F73]/30 rounded-full px-2 py-0.5 tracking-wide">
 <Medal size={10} className="text-[#a78bfa]" />
 <span>Certified Guide</span>
 </span>
 )}
 {post.author?.host_tier === "local_host" && (
 <span className="inline-flex items-center gap-1 text-[9px] font-bold text-[#34e0a1] bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon/10 border border-[#0D6F73]/30 rounded-full px-2 py-0.5 tracking-wide">
 <MapPin size={10} className="text-[#0D6F73]" />
 ...
 <Button size="sm" className="text-xs rounded-full bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon hover:bg-[#6b47d6] text-white" onClick={saveEdit} disabled={saving}>
 ...
 if (urls.length === 1) {
 return (
 <div className="relative overflow-hidden rounded-xl border border-white/5 bg-card shadow-modern aspect-video w-full">
 <PrivatePostImage 
 src={urls[0]} 
 alt="Post media" 
...
export default function FeedPage() {
 ...
 <div className="size-9 rounded-full bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon/10 border border-[#0D6F73]/20 flex items-center justify-center">
 <Rss className="size-4 text-[#0D6F73]" />
 </div>
 <div>
 <h1 className="text-xl font-bold tracking-tight text-white">Community Feed</h1>
 <span className="flex items-center gap-1.5">
 <Globe className="size-3.5 text-[#0D6F73]" />
 Public Shared Space
 </span>
 </div>
 </div>
 ...
 {!currentUserId && (
 <div className="rounded-2xl border border-border/60 bg-card shadow-modern p-4 text-center space-y-2">
 ...
 <button
 onClick={loadFeed}
 className="px-4 py-2 rounded-full bg-carbon-dark shadow-modern-glow border border-brand text-brand hover:bg-carbon text-white text-xs font-semibold hover:bg-[#6b47d6] transition-colors"
 >
 Try Again
 </button>
 </div>
