  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("generate-certificate", {
        body: { 
          name, 
          tree_id: commitmentId, 
          tree_name: "Travel Getaway Sponsorship Pledge", 
          dedication, 
          date 
        }
      })
    ...
    if (printWindow) {
      const localHtml = `
        ...
        <html>
        <head>
          <title>Ausaguide Travel Getaway Commitment Certificate</title>
          <style>
            ...
            .cert-card { text-align: center; border: 2px solid #7F5AF0; border-radius: 20px; padding: 50px 30px; max-width: 650px; background: #121214; }
            h1 { color: #7F5AF0; font-size: 28px; margin-bottom: 20px; text-transform: uppercase; }
        ...
        <body>
          <div class="cert-card">
            ...
            <div style="font-size: 12px; letter-spacing: 2px; color: #7F5AF0; font-weight: bold; text-transform: uppercase;">Certificate of Commitment</div>
            ...
            <div class="meta">
              <div class="meta-item"><div class="meta-label">Commitment ID</div><div class="meta-val" style="color:#7F5AF0;">${commitmentId}</div></div>
    ...
    <div className="relative overflow-hidden min-h-screen bg-[#16161A] text-white flex flex-col items-center">
      ...
      <div className="relative z-10 max-w-2xl w-full px-6 py-16 md:py-24 pt-32 flex flex-col space-y-8 items-center text-center">
        ...
        <div className="space-y-3">
          <div className="mx-auto flex size-14 items-center justify-center rounded-full bg-[#7F5AF0]/10 text-[#7F5AF0] border border-[#7F5AF0]/20 shadow-[0_0_15px_rgba(127,90,240,0.2)]">
            <ShieldCheck className="size-8" />
          </div>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight bg-gradient-to-r from-[#7F5AF0] via-[#2CB67D] to-[#FFFFFE] bg-clip-text text-transparent">
            Pledge Registered!
          </h1>
          <p className="text-sm text-white/60 max-w-md mx-auto">
            Your commitment has been saved in our database. We've reserved commitment ID <span className="font-bold text-[#7F5AF0]">{commitmentId}</span> for you!
          </p>
        </div>
        ...
        >
          <div className="p-8 sm:p-12 bg-radial-gradient border border-border rounded-3xl space-y-8 text-center relative overflow-hidden">
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-48 h-48 bg-[#7F5AF0]/5 rounded-full blur-3xl pointer-events-none" />
            
            <div className="space-y-2">
              <span className="text-3xl">🌅</span>
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#7F5AF0]">
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-2 pt-6 border-t border-border text-left max-w-md mx-auto">
              <div className="space-y-1">
                ...
                <span className="block text-xs font-bold text-[#7F5AF0] font-mono">{commitmentId}</span>
              </div>
              <div className="space-y-1">
