...
export default function Admin2Waitlist() {
 ...
 entries.map(entry => (
 <tr key={entry.id} className="hover:bg-card shadow-modern transition-colors">
 ...
 <span key={i} className="px-2 py-0.5 rounded text-[10px] uppercase tracking-wider bg-card shadow-modern text-gray-300">
 {int.replace('-', ' ')}
 </span>
 ))}
 </div>
 ...
 : "bg-card shadow-modern text-gray-300 hover:bg-card shadow-modern"
 }`}
 >
 {updatingId === entry.id ? (
