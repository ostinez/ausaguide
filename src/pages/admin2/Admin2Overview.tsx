...
export default function Admin2Overview() {
 ...
 disabled={loading}
 className="flex items-center gap-2 px-4 py-2 bg-card shadow-modern hover:bg-card shadow-modern text-white rounded-lg border border-white/10 transition-colors disabled:opacity-50"
 >
 ...
 icon={<Calendar className="text-brand" size={24} />}
 ...
 <div className="p-4 border-b border-white/10 bg-card shadow-modern">
 ...
 <tr key={user.id} className="hover:bg-card shadow-modern">
 <td className="px-4 py-3 text-gray-200">
 <div className="font-medium">{user.full_name}</div>
 <div className="text-xs text-gray-500">{user.email}</div>
 </td>
 <td className="px-4 py-3">
 <span className={`px-2 py-1 rounded-full text-xs font-medium bg-card shadow-modern ${user.role === 'admin' ? 'text-primary' : 'text-gray-300'}`}>
 {user.role || 'none'}
 </span>
 </td>
 ...
 <tr key={booking.id} className="hover:bg-card shadow-modern">
 ...
 <td className="px-4 py-3">
 <span className="px-2 py-1 rounded-full text-xs font-medium bg-card shadow-modern text-gray-300 capitalize">
 {booking.status}
 </span>
 </td>
...
function StatCard({ title, value, icon, subtext, className }: { title: string, value: string, icon: React.ReactNode, subtext?: string, className?: string }) {
 ...
 <div className="p-2 bg-card shadow-modern rounded-lg border border-white/5">
 {icon}
 </div>
 </div>
