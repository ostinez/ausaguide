 ...
 setSubmitting(false)
  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden flex flex-col items-center justify-center py-20 px-6">
      {/* Background blobs */}
      <div className="absolute top-10 left-1/4 h-[300px] w-[300px] rounded-full bg-primary/5 blur-[80px]" />
      <div className="absolute bottom-10 right-1/4 h-[400px] w-[400px] rounded-full bg-primary/5 blur-[100px]" />

      <div className="relative z-10 w-full max-w-lg">
        {success ? (
          <div className="rounded-3xl border border-border p-8 bg-card shadow-modern text-center space-y-6 animate-in fade-in zoom-in duration-300">
            <div className="inline-flex size-16 items-center justify-center rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600">
              <CheckCircle2 className="size-8 animate-pulse" />
            </div>
            <div className="space-y-2">
              <h1 className="text-2xl font-black text-foreground tracking-tight">You're on the Waiting List!</h1>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Thank you for applying to host with us! We have sent a confirmation email to <span className="text-primary font-bold">{email}</span>. We'll be in touch as soon as we launch hosting in your area.
              </p>
            </div>
            <button
              onClick={() => window.location.href = "/"}
              className="w-full py-3 rounded-full bg-primary text-primary-foreground text-sm font-bold shadow-md hover:bg-primary/90 transition duration-300"
            >
              Return Home
            </button>
          </div>
          ...
          <div className="rounded-3xl border border-border p-8 bg-card shadow-modern space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-card border border-border rounded-xl pl-11 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-foreground uppercase tracking-wider">Phone Number (Optional)</label>
                <div className="relative">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-foreground uppercase tracking-wider">Where are you located? *</label>
                <div className="relative">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-foreground uppercase tracking-wider">Why do you want to host? *</label>
                <div className="relative">
                  ...
                  <textarea
                    ...
                    className="w-full bg-card border border-border rounded-xl pl-11 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors resize-none"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full mt-2 py-3 rounded-full bg-primary text-primary-foreground text-sm font-bold shadow-md hover:bg-primary/90 disabled:opacity-50 transition duration-300 flex items-center justify-center gap-2"
              >
                {submitting ? (
                  <>
